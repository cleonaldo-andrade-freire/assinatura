...
- **Mini-CRM de leads com triagem por IA no WhatsApp** (`/dashboard/leads`) — número sem anamnese em andamento nem agendamento pendente deixou de ser só ignorado: um agente (Claude Haiku, via Vercel AI SDK) assume a conversa, oferece horário e agenda direto (caso eletivo) ou marca como urgente pra recepção assumir. Ver `app/README.md` seção "Mini-CRM de leads".
- **Contra-assinatura da dentista com certificado ICP-Brasil, num único PDF** (anamnese e evolução clínica) — antes gerava um segundo arquivo separado, sem nenhum aviso ao paciente; agora mescla no mesmo PDF que o paciente assinou e notifica por WhatsApp com link de validação pública (`/validar-anamnese`, `/validar-evolucao`). Ver `app/README.md` seção "Contra-assinatura da dentista".
